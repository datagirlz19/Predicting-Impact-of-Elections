...
ggplot(Final1, aes(x= Candidate,y= No_of_states, fill= Candidate))+geom_bar(stat="identity")
...
ggplot(Final, aes(x=Publicity, y=Votes, color=Candidate, label=Abbr))+geom_point()+geom_smooth(method=lm, se=FALSE)
...
ggplot(Final2, aes(x=mean_unemp, y=Votes, color=Candidate, label=Abbr))+geom_point()+geom_text(hjust=0, vjust=0)+geom_line()
ggplot(Final2, aes(x=mean_unemp, y=Votes, color=Candidate))+geom_point()+geom_smooth(method = lm, se = FALSE)


Hate=Final%>%select(State, Abbr, Candidate, Hate_Crimes, Votes)
Hate=Hate%>%filter(State!="District of Columbia")
#ggplot(Hate, aes(x=Hate_Crimes, y=Votes, color=Candidate))+geom_point()+geom_text(hjust=0, vjust=0)+geom_smooth(method=lm, se=FALSE)
ggplot(Hate, aes(x=Hate_Crimes, y=Votes, color=Candidate, label=Abbr))+geom_point()+geom_text(hjust=0,vjust=0)+geom_smooth(method = lm, se=FALSE)
